...
from data_loaders.dataloader import load_datavis as load_data, TestDataset

from human_body_prior.body_model.body_model import BodyModel as BM
from tqdm import tqdm

from utils import utils_transform, utils_teasor as utils_visualize_my
        ...
        with torch.no_grad():
            body_pose = self.body_model(
                **{
                    ...
                    if k in ["pose_body", "trans", "root_orient"]#just take out these three keys
                }
            )
        return body_pose
...
def non_overlapping_test(
    ...
    flag =torch.zeros(sparse_original.shape[1])
    ...
    for i in range (num_frames):
        if torch.all(sparse_original[0][i]== 0):
            flag[i]=1
            if (all(flag[j]==1 for j in range(i)) or  (i==0)):
                next_idx = min(j for j in  range(i + 1, num_frames) if not torch.all(sparse_original[0][j]== 0))
                print('nextIdx',next_idx)
                sparse_original[i, :] = sparse_original[next_idx, :]
            else:
                prev_idx = max(j for j in range(i) if flag[j]==0)
                sparse_original[i, :] = sparse_original[prev_idx, :]
    output_samples = []
    count = 0
    sparse_splits = []
    flag_index = None
...
def overlapping_test(
    ...
    return output_samples, body_param, head_motion, filename,all_frames
...
def evaluate_prediction(
    ...
    #print(f"motion_pred shape before processing: {motion_pred.shape}")
    model_rot_input = (
        utils_transform.sixd2aa(motion_pred.reshape(-1, 6).detach())
        .reshape(motion_pred.shape[0], -1)
        .float()
    )
    #print("model_rot_input shape",model_rot_input.shape)  ##############
    ...
    #print("body_pose_local shape",body_pose_local.shape)  ##############

    # Get the offset in global coordiante system between head and body_world.
    t_head2root = -body_pose_local[:, 15, :]
    #print("t_head2root shape", t_head2root.shape)  ##############
    #print("t_head2world shape", t_head2world.shape)  ##############11111111111111################
    ...
    if args.vis:
        ...
        utils_visualize_my.save_animation(
...
def main():
    ...
    if not args.overlapping_test:
        ...
        #print("n_testframe", n_testframe)  ##############
    total_time = 0.0
    all_frames = 0
    for sample_index in tqdm(range(len(dataset))):
        start_time = time.time()
        output, body_param, head_motion, filename, all_frames = test_func(
            args,
        ...
        #print("head_motion shape", head_motion.shape)  ##############
    ...
    print(f"single time: {total_time/all_frames:.5f}s")
