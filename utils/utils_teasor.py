...
class CheckerBoard:
    def __init__(self, white=(247, 246, 244), black=(146, 163, 171)):
        self.white = np.array([0.98, 0.98, 0.98, 1.])  # np.array(white) / 255.0
        self.black = np.array([0.777, 0.777, 0.777, 1.])  # p.array(black) / 255.0
        self.verts, self.faces, self.texts = None, None, None
        self.offset = None

    @staticmethod
    def gen_checker_xy(black, white, square_size=1, xlength=250.0, ylength=50.0):
        """
        ...
        mesh = trimesh.Trimesh(
            vertices=np.array(verts) + np.array([-8, -5, 0]), faces=np.array(faces), process=False,
...
def get_vector_mesh(sparse_mat):
    mesh_head = trimesh.creation.axis(origin_size=0.05, axis_length=0.15)
    mesh_lh = trimesh.creation.axis(origin_size=0.05, axis_length=0.15)
    mesh_rh = trimesh.creation.axis(origin_size=0.05, axis_length=0.15)
...
def save_animation(body_pose, savepath, bm, color_map=None, fps=60, resolution=(1800, 800)):
    imw, imh = resolution
    ...
    checker_mesh.apply_transform(
        trimesh.transformations.rotation_matrix(0, (0, 0, 10))
    )
    checker_mesh.apply_transform(
        trimesh.transformations.rotation_matrix(30, (10, 0, 0))
    )
    checker_mesh.apply_transform(
        trimesh.transformations.translation_matrix([-100, 0, -4])
    )
    ...
    add_axis = True
    vert_color = np.tile(colors["purple"]+[.5], (6890, 1))
    ...
    for fId in all_item:
        ...
        body_mesh.apply_transform(
            trimesh.transformations.rotation_matrix(np.radians(280), (0, 0, 10))
        )
        body_mesh.apply_transform(
            trimesh.transformations.rotation_matrix(np.radians(-82), (10, 0, 0))
        )
        body_mesh.apply_transform(
            trimesh.transformations.translation_matrix([0, -1, 0])
        )
        ...
        for i in range(22):
            ...
            joint_mesh.apply_transform(trimesh.transformations.translation_matrix([0, -1, 0]))
        ...
        for line in line_between_point_upper:
            ...
            cylinder.apply_transform(trimesh.transformations.translation_matrix([0, -1, 0]))
        ...
        for line in line_between_point_lower:
            ...
            line_list.append(cylinder)
        '''
        if begin_frame + 60 < fId < end_frame:
            mesh_vert_list = body_vert_list[fId - begin_frame - 60:fId - begin_frame]
        '''        
        if add_axis:
        ...
        if begin_frame + 60 < fId < end_frame:
            ...
            prev_second_vert = body_vert_list[fId - begin_frame - 20]
            prev_third_vert = body_vert_list[fId - begin_frame - 30]
            prev_fourth_vert = body_vert_list[fId - begin_frame - 40]
            prev_fifth_vert = body_vert_list[fId - begin_frame - 50]
            #prev_sixth_vert = body_vert_list[fId - begin_frame - 60]
            #prev_seventh_vert = body_vert_list[fId - begin_frame - 70]
            mesh_vert_list = [prev_first_vert, prev_second_vert, prev_third_vert, 
                              prev_fourth_vert, prev_fifth_vert]
            transparency = [.8, .8, .8, .8, .8, .8, .8]
            body_mesh_tmp_list = []
            for i in range(5):
                cur_mesh = trimesh.Trimesh(
                    vertices=body_vert_list[fId - begin_frame - 10*i-10],
                    faces=faces,
                    # vertex_colors=color_map[fId],
                ...
                cur_mesh.apply_transform(
                    trimesh.transformations.rotation_matrix(rotate_angle+0.5+3.14, (0, 0, 10))
                )
                cur_mesh.apply_transform(
                    trimesh.transformations.rotation_matrix(30, (10, 0, 0))
                )
                cur_mesh.apply_transform(
                    trimesh.transformations.translation_matrix([-1.2+0.8*i, -0.1, -1])
                )
                ...
                for j in range(22):
                    ...
                    trasform_joint = torch.eye(4)
                    transition = c2c(body_pose.Jtr[fId-10*i-10, j])
                ...
                if add_axis:
                    sparse_angle = body_pose.full_pose[fId -10 * (i + 1), :66].reshape(22, 3)[[15, 20, 21]]  # (3, 3)
                    sparse_matrix = utils_transform.aa2matrot(sparse_angle)  # (3, 3, 3)
                    sparse_pos = body_pose.Jtr[fId - 10 * (i + 1), :22][[15, 20, 21]]  # (3, 3)
                    mat_all = torch.cat((sparse_matrix, sparse_pos[..., None]), dim=-1)  # (3, 3, 4)
                    last_row = torch.tensor([0., 0., 0., 1.]).reshape(1, 1, 4).repeat(3, 1, 1).to(mat_all.device)
                    mat_all = torch.cat((mat_all, last_row), dim=1)
                    orien_mesh_list_tmp = get_vector_mesh(mat_all.cpu())
                    for orien_mesh in orien_mesh_list_tmp:
                        ...
                        orien_mesh.apply_transform(trimesh.transformations.scale_matrix(scale_fac))
                    orien_mesh_list += orien_mesh_list_tmp
            mv.set_static_meshes([checker_mesh,body_mesh] + body_mesh_tmp_list + orien_mesh_list+joint_mesh_list + line_list)  # orien_mesh_list
        else:
            # mv.set_static_meshes([checker_mesh, body_mesh])
            mv.set_static_meshes([body_mesh] + orien_mesh_list)  # orien_mesh_list

        #mv.set_static_meshes([body_mesh] +joint_mesh_list + line_list + orien_mesh_list)
    ...
    img_save_dir = f'{ os.path.dirname(savepath)}/{file_name}'
