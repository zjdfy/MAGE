...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        n, c = sparse.shape
        num_masked_frames = int(n * 0.1)
        masked_indices = random.sample(range(n), num_masked_frames)
        sparse[masked_indices, :] = 0  # 遮掩对应帧
        '''
...
def get_pathvis(dataset_path, split):
    data_list_path = []
    files = glob.glob(dataset_path +"/CMU/" + split + "/55.pt")
...
def load_datavis(dataset, dataset_path, split, **kwargs):
   
    if split == "test":
        motion_list = get_pathvis(dataset_path, split)
